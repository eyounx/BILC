...
class RacosOptimization:
    ...
    def Initialize(self, func,data=None):
        temp = []
        ...
        for i in range(self.__SampleSize+self.__PositiveNum):
            ...
            while(True):
                ...
                if self.InstanceInList(ins, temp, i) is False:
                    break
            ins.setFitness(func(ins.getFeatures(),data))
            temp.append(ins)
        # sorted by fitness
    ...
    param:
        ...
        ub:   uncertain bits
    '''
    def ContinueOpt(self, func, ss, mt, pn, rp, ub, data=None):

        self.Clear()
        self.setParameters(ss, mt, pn, rp, ub)
        self.ResetModel()
        self.Initialize(func, data)
        ...
        if self.__OnlineSwitch is False:
            ...
            for itera in range(self.__MaxIteration-1):
                ...
                self.UpdatePosPop()
                self.UpdateOptimal()
                print self.getOptimal().getFitness()
        else:
            #Online style
